...
export const registerGenero = async (req, res) => {
    try {
        ...
        res.status(201).json({ message: "Género y subgéneros registrados con éxito.", redirect: "/generos" });
    } catch (error) {
        ...
        const generos = result.recordset.reduce((acc, row) => {
            ...
            if (generoIndex === -1) {
                acc.push({
                    ...
                    subgeneros: row.ID_SBC ? [{ ID_SBC: row.ID_SBC, NOMBRE_SBC: row.NOMBRE_SBC }] : []
                });
            } else if (row.ID_SBC) {
...
export const getGeneroById = async (req, res) => {
    try {
        ...
        const generoResult = await pool.request().input("ID_LCAT", sql.Int, id).query(`
        ...
        const subgenerosResult = await pool.request().input("ID_LCAT", sql.Int, id).query(`
...
export const editGenero = async (req, res) => {
    try {
        ...
        // 🔥 Verifica si `subgeneros` es un string JSON válido antes de parsearlo
        let subgenerosArray = [];
        if (subgeneros) {
            try {
        ...
        // 🔥 Verifica si el género existe antes de actualizarlo
        const checkGenero = await pool.request()
        ...
        await pool.request()
            ...
            .input("NOMBRE_LCAT", sql.NVarChar(300), NOMBRE_LCAT)
            .input("DESCRIPCION_LCAT", sql.NVarChar(300), DESCRIPCION_LCAT)
            .input("ACTIVO_LCAT", sql.Bit, ACTIVO_LCAT ? 1 : 0)
            .query(`
        ...
        // 🔥 Manejar subgéneros
        await pool.request().input("ID_LCAT", sql.Int, id).query(`DELETE FROM SUBCATEGORIA_LCAT_T WHERE ID_LCAT = @ID_LCAT`);

        for (const subgenero of subgenerosArray) {
            await pool.request()
                .input("ID_LCAT", sql.Int, id)
                .input("NOMBRE_SBC", sql.NVarChar(300), subgenero)
                .query(`INSERT INTO SUBCATEGORIA_LCAT_T (ID_LCAT, NOMBRE_SBC) VALUES (@ID_LCAT, @NOMBRE_SBC)`);
        }
